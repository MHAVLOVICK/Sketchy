...
public class GetDrawingSettings extends ServletAction {
	@Override
	public JSONServletResult execute(HttpServletRequest request) throws Exception {
		...
		try{
			DrawingProperties drawingProperties = SketchyContext.drawingProperties;
			if (drawingProperties==null){
				drawingProperties = new DrawingProperties();
			}
			jsonServletResult.put("properties", drawingProperties);
		} catch (Throwable t){
			jsonServletResult = new JSONServletResult(Status.ERROR, "Error Getting Drawing Properties! " + t.getMessage());
		}
