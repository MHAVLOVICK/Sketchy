...
public class SaveDrawingSettings extends ServletAction {
	@Override
	public JSONServletResult execute(HttpServletRequest request) throws Exception {
		...
		try{
			...
			SketchyContext.drawingProperties = drawingProperties;
			
			for (String drawingSize:drawingProperties.getDrawingSizes()){
				// parse just to make sure it's a valid Drawing Size Entry
				DrawingSize.parse(drawingSize);
			}
