...
public class ChangeSpeed extends ServletAction {
	@Override
	public JSONServletResult execute(HttpServletRequest request) throws Exception {
		...
		try{
			String drawSpeedString = request.getParameter("drawSpeed");
			int drawSpeed = 0;
			try{
				drawSpeed=Integer.parseInt(drawSpeedString);
			} catch (Exception e){}
			
			if (drawSpeed>0){
				plotterControllerProperties.setDrawSpeed(drawSpeed);
			}
			
			String moveSpeedString = request.getParameter("moveSpeed");
			int moveSpeed = 0;
			try{
				moveSpeed=Integer.parseInt(moveSpeedString);
			} catch (Exception e){}
			
			if (moveSpeed>0){
				plotterControllerProperties.setMoveSpeed(moveSpeed);
			}
